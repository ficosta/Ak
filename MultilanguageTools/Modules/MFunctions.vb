...
Module MFuncions
  ...
  Public Function SerializeObjectToFile(ByVal siFile As String, ByRef CiObject As Object, Optional type As eSerializationType = eSerializationType.Xml) As Boolean

    Dim bRes As Boolean = False
    Try
      Dim sRes As String = SerializeObjectToString(CiObject, type)
      If sRes <> "" Then
  ...
  Public Function SaveStringToFile(ByVal siFile As String, siData As String) As Boolean
    Dim bRes As Boolean = False
    Try
      Dim myFileStream As FileStream = File.Create(siFile)
      Dim bytes() As Byte
  ...
  Public Function LoadStringFromFile(ByVal siFile As String) As String
    Dim sRes As String = ""
    Try
  ...
  Public Function SerializeObjectToString(ByRef CiObject As Object, Optional type As eSerializationType = eSerializationType.Xml) As String
    Dim sRes As String = ""
    Try
      Select Case type
        Case eSerializationType.json
          sRes = Newtonsoft.Json.JsonConvert.SerializeObject(CiObject)
          sRes = Newtonsoft.Json.Linq.JValue.Parse(sRes).ToString(Newtonsoft.Json.Formatting.Indented)
  ...
  Public Function DesserializeObjectFromFile(ByVal siFile As String, ByRef CoObject As Object, Optional type As eSerializationType = eSerializationType.Xml) As Boolean
    Dim bRes As Boolean = False
    Try
      If File.Exists(siFile) Then
        Dim myFileStream As FileStream = File.OpenRead(siFile)
        ...
        Dim sRes As String
        sRes = System.Text.Encoding.UTF8.GetString(bytes)
        DesserializeObjectFromString(sRes, CoObject, type)
        myFileStream.Close()
        bRes = True
      End If
    Catch ex As Exception
  ...
  Public Function DesserializeObjectFromString(ByVal siString As String, ByRef CoObject As Object, Optional type As eSerializationType = eSerializationType.Xml) As Boolean
    Dim bRes As Boolean = False
    ...
    Try
      ...
      Select Case type
        ...
        Case eSerializationType.json
          Dim myType As System.Type = CoObject.GetType
          CoObject = Newtonsoft.Json.JsonConvert.DeserializeObject(siString)
      End Select
      bRes = True
    Catch ex As Exception
    End Try
  ...
  Public Function DesserializeObjectFromFile(Of T As Class)(ByVal siFile As String, ByRef CoObject As Object, Optional type As eSerializationType = eSerializationType.Xml) As Boolean
    Dim bRes As Boolean = False
    Try
      If File.Exists(siFile) Then
        Dim myFileStream As FileStream = File.OpenRead(siFile)
        ...
        Dim sRes As String
        sRes = System.Text.Encoding.UTF8.GetString(bytes)
        DesserializeObjectFromString(Of T)(sRes, CoObject, type)
        myFileStream.Close()
        bRes = True
      End If
    Catch ex As Exception
  ...
  Public Function DesserializeObjectFromString(Of T As Class)(ByVal siString As String, ByRef CoObject As Object, Optional type As eSerializationType = eSerializationType.Xml) As Boolean
    Dim bRes As Boolean = False
    Dim sRead As String = siString
    Try
      If sRead.Contains(vbNullChar) Then
      ...
      Select Case type
        ...
        Case eSerializationType.json
          Dim myType As System.Type = CoObject.GetType
          CoObject = Newtonsoft.Json.JsonConvert.DeserializeObject(Of T)(siString)
      End Select
      bRes = True
    Catch ex As Exception
    End Try
